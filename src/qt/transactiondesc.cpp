...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (wtx.IsCoinBase() || wtx.IsCoinStake())
        strHTML += (wtx.GetBlocksToMaturity() > 0 ? "<br>" + tr("Generated coins must mature ") + tr("%n", "", wtx.GetBlocksToMaturity()) + tr(" blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.") + "<br>" :
                                                    "<br>" + tr("Generated coins matured and can be spent.") + "<br>");
    //
    // Debug view
    //
    if (fDebug)
