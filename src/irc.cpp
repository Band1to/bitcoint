...
void ThreadIRCSeed2()
{
    ...
    while (!ShutdownRequested())
    {
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #bitcointTEST\r");
            Send(hSocket, "WHO #bitcointTEST\r");
        } else
        {
            LogPrintf("JOIN #bitcoint\n");
            // randomly join #bitcoint00-#bitcoint05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bitcoint%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bitcoint%02d\r", channel_number).c_str());
        };
