...
#define _STR(d) dynamic_cast< std::ostringstream & >((std::ostringstream() << std::dec << d)).str()
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bitcoint.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bitcointd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: ") + _STR(GetParams(CChainParams::MAIN).GetDefaultPort()) + _(" or testnet: ") + _STR(GetParams(CChainParams::TESTNET).GetDefaultPort()) + _(")") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: ") + _STR(GetParams(CChainParams::MAIN).RPCPort()) + _(" or testnet: ") + _STR(GetParams(CChainParams::TESTNET).RPCPort()) + _(")") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitcoinT is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitcoinT is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitcoinT version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "BitcoinT server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitcoinT") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BitcoinT to complete") << "\n";
