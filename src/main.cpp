...
const string strMessageMagic = "BitcoinT Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    int nMaturity = IsSuperStake() ? Params().CoinbaseMaturitySuperStake() : Params().CoinbaseMaturity();
...
int generateMTRandom(unsigned int nSeed, int nRange)
{
    random::mt19937 gen(nSeed);
    random::uniform_int_distribution<> dist(1, nRange);
    return dist(gen);
}

int64_t GetLastSuperStakeTime(CBlockIndex *pindex, unsigned int nSuperStake)
{
    CBlockIndex *tpindex = pindex;
    while (tpindex != NULL && (tpindex->nTime > Params().SuperStakeStartTime())) {
        if (tpindex->IsSuperStake(nSuperStake))
            return tpindex->nTime;
        tpindex = tpindex->pprev;
    }

    return Params().SuperStakeStartTime();
}

static inline bool inRange(int64_t nValue, int64_t nMinValue, int64_t nMaxValue) { return ((nValue >= nMinValue) && (nValue <= nMaxValue)); }
...
bool SuperStakeReward(const CBlockIndex* pindexPrev, int64_t nTotalValueIn, unsigned int nSuperStake, int64_t &nSubsidy)
{
    int64_t nLastSuperStakeTime = GetLastSuperStakeTime(pindexBest, nSuperStake);
    int64_t nLastSuperStakeSpan = nLastSuperStakeTime / Params().SuperStakeTimespan(nSuperStake);
    int64_t nPrevHeightTime = pindexPrev->nTime;
    int64_t nPrevHeightSpan = nPrevHeightTime / Params().SuperStakeTimespan(nSuperStake);

    if (fDebug)
    {
        switch (nSuperStake) {
        case CChainParams::SUPER_STAKE_5K:
            LogPrintf("SuperStake 5K\n");
            LogPrintf("-------------------------------------------------\n");
            break;
        case CChainParams::SUPER_STAKE_15K:
            LogPrintf("SuperStake 15K\n");
            LogPrintf("-------------------------------------------------\n");
            break;
        case CChainParams::SUPER_STAKE_60K:
            LogPrintf("SuperStake 60K\n");
            LogPrintf("-------------------------------------------------\n");
            break;
        default:
            break;
    ...
    LogPrintf("nLastSuperStakeTime   : %d\n", nLastSuperStakeTime);
    LogPrintf("nPrevHeightTime       : %d\n", nPrevHeightTime);
    LogPrintf("nLastSuperStakeSpan   : %d\n", nLastSuperStakeSpan);
    LogPrintf("nPrevHeightSpan       : %d\n", nPrevHeightSpan);
    LogPrintf("nTotalValueIn         : %d\n", nTotalValueIn);
    LogPrintf("nSuperStakeMinValueIn : %d\n", Params().SuperStakeMinValueIn(nSuperStake));
    LogPrintf("nSuperStakeReward     : %d\n", Params().SuperStakeReward(nSuperStake));

    if ((nPrevHeightSpan > nLastSuperStakeSpan) && (nTotalValueIn >= Params().SuperStakeMinValueIn(nSuperStake))) // Minimum stake coins rule
    {
        uint256 hash = pindexPrev->GetBlockHash();
        std::string cSeed_str = hash.ToString().substr(12,7);
        const char* cSeed = cSeed_str.c_str();
        long nSeed = hex2long(cSeed);
        //int64_t nRand = generateMTRandom(nSeed, 100000);

        int64_t nRandspan = Params().SuperStakeTimespan(nSuperStake) * 10e3;
        int64_t nSuperspan = std::min((nPrevHeightSpan - nLastSuperStakeSpan) * (nPrevHeightSpan - nLastSuperStakeSpan) * (nRandspan / 50), nRandspan); // 100 - 1%, 50 - 2%, 25 - 4% etc

        int64_t nRand = generateMTRandom(nSeed, nRandspan);

        int64_t nAvgBlocks = (pindexPrev->nTime - GetLastSuperStakeTime(pindexBest, nSuperStake)) / Params().TargetSpacing();
        int64_t nTarget = 5 * nAvgBlocks * nAvgBlocks;

        LogPrintf("nAvgBlocks            : %015d\n", nAvgBlocks);
        LogPrintf("nTarget               : %015d\n", nTarget);
        LogPrintf("nRandspan             : %015d\n", nRandspan);
        LogPrintf("nSuperspan            : %015d\n", nSuperspan);
        LogPrintf("nRand                 : %015d\n", nRand);
        LogPrintf("min                   : %015d\n", (nRandspan - nSuperspan) / 2);
        LogPrintf("max                   : %015d\n", (nRandspan + nSuperspan) / 2);

        if (inRange(nRand, (nRandspan - nSuperspan) / 2, (nRandspan + nSuperspan) / 2) && (Params().SuperStakeReward(nSuperStake) > nSubsidy)) // Accepts only if SuperStakeReward greater than calculated nSubsidy
        {
            nSubsidy = Params().SuperStakeReward(nSuperStake);

            LogPrintf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> SUPERSTAKE : %ld >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n", nSubsidy);
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nTotalValueIn, int64_t nFees, unsigned int &nSuperStake)
{
    int64_t nRewardCoinYear = Params().PoSCoinReward();
    int64_t nSubsidy = nCoinAge * nRewardCoinYear * 33 / (365 * 33 + 8);

    if (isSuperStakeTime(GetAdjustedTime()))
    {
        if (SuperStakeReward(pindexPrev, nTotalValueIn, CChainParams::SUPER_STAKE_5K, nSubsidy))
            nSuperStake = CChainParams::SUPER_STAKE_5K;
        else if (SuperStakeReward(pindexPrev, nTotalValueIn, CChainParams::SUPER_STAKE_15K, nSubsidy))
            nSuperStake = CChainParams::SUPER_STAKE_15K;
        else if (SuperStakeReward(pindexPrev, nTotalValueIn, CChainParams::SUPER_STAKE_60K, nSubsidy))
            nSuperStake = CChainParams::SUPER_STAKE_60K;
    }
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (fDebug)
    {
        LogPrintf("GetNextTargetRequired RETARGET\n");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            int nMaturity = txPrev.IsSuperStake() ? Params().CoinbaseMaturitySuperStake() : Params().CoinbaseMaturity();
            if (txPrev.IsCoinBase() || txPrev.IsCoinStake())
                for (const CBlockIndex* pindex = pindexBlock; pindex && pindexBlock->nHeight - pindex->nHeight < nMaturity; pindex = pindex->pprev)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            nValueIn += nTxValueIn;
            if (IsValidTxAmount(nTxValueIn, nTxValueOut))
        ...
        if (tx.IsCoinStake())
        {
            ...
            //revert nFees(we added (-reward) as fee in prev block)
            nFees += nStakeReward;
        }
    ...
    if (IsProofOfStake())
    {
        ...
        uint64_t nCoinAge;
        int64_t nTotalValueIn;

        if (!vtx[1].GetCoinAge(txdb, nCoinAge, nTotalValueIn))
            return error("ConnectBlock() : %s unable to get coin age for coinstake", vtx[1].GetHash().ToString());

        nCalculatedStakeReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nTotalValueIn, nFees, nSuperStake);

        if (nStakeReward > nCalculatedStakeReward)
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%d vs calculated=%d)", nStakeReward, nCalculatedStakeReward));

        if (nSuperStake and !vtx[1].IsSuperStake())
            return DoS(100, error("ConnectBlock() : superstake tries to pass as usual block"));
    ...
    if (nSuperStake)
        pindex->SetSuperStake(nSuperStake);
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge, int64_t& nTotalValueIn) const
{
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // Check proof of work matches launch time
    if (GetAdjustedTime() < Params().LaunchTime())
        return DoS(50, error("CheckBlock() : block is too early"));
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // SUPERSTAKE time validity
        //if (tx.IsSuperStake() && !isSuperStakeTime(GetAdjustedTime()))
          //  return DoS(50, error("CheckBlock() : super stake block is out of time bounds"));
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfStake() && nHeight < Params().PoSSwitch())
