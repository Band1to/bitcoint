...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    CTxDB txdb("r");
    if (!tx.GetCoinAge(txdb, nCoinAge, nTotalValueIn))
        throw JSONRPCError(RPC_MISC_ERROR, "GetCoinAge failed");
    
    return (uint64_t)GetProofOfStakeReward(pindexPrev, nCoinAge, nTotalValueIn, 0, nSuperStake);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BitcoinT is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BitcoinT is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BitcoinT is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BitcoinT is downloading blocks...");
