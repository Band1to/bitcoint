    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s BT %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s BT %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        CTxDB txdb("r");
        if (!txNew.GetCoinAge(txdb, nCoinAge, nTotalValueIn))
            return error("CreateCoinStake : failed to calculate coin age");
        
        unsigned int fSuperStake = 0;
        int64_t nReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nTotalValueIn, nFees, fSuperStake);
        if (nReward <= 0)
            return false;

        if (fSuperStake)
            txNew = txNew.SetSuperStake(txNew);
